(function(window){
    ...
    Heart = function() {
        ...
        var points = [], x, y, t;
        for (var i = 50; i < 70; i += 0.2) {
            t = i / Math.PI;
            x = 26 * Math.pow(Math.sin(t), 3);
            y = 21 * Math.cos(t) - 5 * Math.cos(2 * t) - 2 * Math.cos(3 * t) - Math.cos(4 * t);
    ...
    Seed.prototype = {
        ...
        drawText: function() {
            ...
            ctx.moveTo(0, 0);
            ctx.scale(0.75, 0.75);
            ctx.font = "12px 微软雅黑"; // 字号肿么没有用? (ˉ(∞)ˉ)
            ctx.fillText("点❤有惊喜哟", 33, 10);
            ctx.restore();
        },
        clear: function() {
            var ctx = this.tree.ctx, cirle = this.cirle;
            ...
            ctx.clearRect(point.x - w, point.y - h, 4.5 * w, 4.5 * h);
        },
        hover: function(x, y) {
